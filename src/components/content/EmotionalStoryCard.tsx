...
import { Play, Pause, FileText, AudioLines } from "lucide-react";
import { cn } from "@/lib/utils";
import { motion } from "framer-motion";
import { Badge } from "@/components/ui/badge";
import { EmotionTag } from "../emotion-detection/EmotionTag";
import { EmotionType } from "../emotion-detection/types";

export type StoryType = "audio" | "photo" | "transcript";

export interface EmotionalStoryCardProps {
  id: string;
  title: string;
  content: string;
  type: StoryType;
...
export const EmotionalStoryCard = ({
  id,
  title,
  content,
  type,
  ...
  const renderCardIcon = () => {
    switch (type) {
      ...
      case "photo":
        return <img src={imageUrl} alt={title} className="h-5 w-5 rounded-full object-cover" />;
      case "transcript":
