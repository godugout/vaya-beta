...
const HanumanResourcesSidebar: React.FC = () => {
  ...
  return (
    <div className="backdrop-blur-sm bg-gradient-to-br from-amber-900/20 via-amber-800/10 to-green-900/15 rounded-3xl shadow-[0_0_40px_rgba(255,126,0,0.15)] p-5 h-full overflow-hidden border-none relative">
      ...
      <div className="space-y-2.5 relative">
        {resources.map((resource, index) => (
          <motion.div 
            ...
            whileTap={{ scale: 0.98 }}
          >
            <button className="w-full p-3 rounded-xl transition-all duration-300 bg-black/20 hover:bg-black/30 border border-white/5 text-left">
              <div className="flex gap-2.5 items-start">
                <div className="p-1.5 rounded-lg bg-gradient-to-br from-hanuman-primary/30 to-hanuman-saffron/30 text-hanuman-gold">
                  {resource.icon}
                </div>
                <div>
                  <h3 className="font-medium text-sm text-white/90">{resource.title}</h3>
                  <p className="text-xs text-white/60 mt-0.5">{resource.description}</p>
                </div>
              </div>
            </button>
          </motion.div>
      ...
      <div className="mt-4 p-3 rounded-xl bg-gradient-to-r from-hanuman-cosmic-blue/30 to-hanuman-cosmic-purple/30 border border-hanuman-gold/10">
        <h3 className="text-xs font-medium text-hanuman-gold mb-1">Family Archives</h3>
